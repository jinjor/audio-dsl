// From https://github.com/jinjor/typed-parser
...
class ParseErrorImpl extends Error implements ParseError {
  ...
  private getPosition(offset: number): Position {
    ...
    return this.positions.get(offset) as any;
  }
  explain(): string {
    let text = "";
...
class Context {
  offset: number = 0;
  scope: Scope = new Scope(0, null as any);
}
...
export function withContext<A>(name: string, parser: Parser<A>): Parser<A> {
  return (source, context) => {
    ...
    context.scope = context.scope.parent as any;
    return result;
  };
}
