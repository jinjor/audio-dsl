...
describe("Validate", function () {
  ...
  it("built-in functions", () => {
    ...
    assertOk(`void f() { float a = floor(0.0); }`);
    assertOk(`void f() { float a = trunc(0.0); }`);
    assertOk(`void f() { float a = nearest(0.0); }`);
    ...
    assertErrorExists(`void f() { float a = to_int(0.0); }`);
    assertErrorExists(`void f() { int a = to_float(0); }`);
    assertErrorExists(`void f() { int a = to_int(0); }`);
    assertErrorExists(`void f() { int a = to_float(0.0); }`);
    assertErrorExists(`void f() { float a = to_int(0); }`);
    assertErrorExists(`void f() { int a = to_int(0.0, 0.0); }`);
    assertErrorExists(`void f() { int a = to_int(); }`);
